            ...
            for (var scan = ex; scan != null; scan = scan.InnerException)
            {
                ...
                yield return new ErrorDetails
                {
                    ...
                    StackFrames = StackTraceHelper.GetFrames(ex)
                        .Select(frame => GetStackFrameInfo(frame.Method, frame.FilePath, frame.LineNumber))
                };
            };
        }

        // make it internal to enable unit testing
        internal StackFrameInfo GetStackFrameInfo(string method, string filePath, int lineNumber)
        {
